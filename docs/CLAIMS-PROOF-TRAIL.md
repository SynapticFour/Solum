...
| A6 | HELIOS-**oriented** audit export envelope exists; chain verifies | Worked example step 9 / standalone `audit export` + `audit verify` | `helios-export.json` / chain `solum-audit-helios-chain-v1` | In-process HELIOS signing inside Solum binary (use external `helios solum-audit` — [helios.md](helios.md)) |
| A7 | EU profile residency / custody check | `SOLUM_STORAGE_REGION=EU cargo run -p solum-core -- check --profile config/profiles/eu-ehds.toml` · wrong region → non-zero ([profiles.md](profiles.md)) | CLI `ok:` / refuse message | Legal compliance certificate |

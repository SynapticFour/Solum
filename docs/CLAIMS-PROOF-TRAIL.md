...
| A9 | IPS-oriented Bundle structural PASS | [FHIR-VALIDATION.md](FHIR-VALIDATION.md) · `./scripts/validate-fhir-ips.sh` | `examples/fhir-ips-export/out/structural-check.txt` | ISiK / TI readiness |
| A10 | Same Bundle + HL7 Validator + `hl7.fhir.uv.ips#2.0.0` → 0 errors / 0 warnings (pinned campaign) | JAR in `.cache/`; `FHIR_VALIDATOR_JAR=… ./scripts/validate-fhir-ips.sh` | `out/validator-log.txt` Success | Full IPS certification beyond package pin; clinical correctness; product `solum fhir` CLI |

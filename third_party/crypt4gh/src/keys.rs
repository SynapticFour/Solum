...
fn decipher(ciphername: &str, data: &[u8], private_ciphertext: &[u8]) -> Result<Vec<u8>, Crypt4GHError> {
	...
	let mut output = private_ciphertext.to_vec();

	assert!((private_ciphertext.len() % block_size(ciphername)?) == 0);

	// Decipher (OpenSSH encrypted private keys only; Ferrum uses unencrypted Crypt4GH keys in production).
	match ciphername {
		"aes128-ctr" => {
		...
		"aes192-ctr" => {
			let mut cipher = Ctr64BE::<Aes192>::new(key.into(), iv.into());
			cipher.apply_keystream(&mut output);
		},
		"aes256-ctr" => {
		...
		"aes128-cbc" => {
			...
			let dec = CbcDecryptor::<Aes128>::new_from_slices(key, iv)
				.map_err(|e| Crypt4GHError::DecryptKeyError(e.to_string()))?;
		...
		"aes192-cbc" => {
			let mut buf = output.clone();
			let dec = CbcDecryptor::<Aes192>::new_from_slices(key, iv)
				.map_err(|e| Crypt4GHError::DecryptKeyError(e.to_string()))?;
			dec.decrypt_padded_mut::<NoPadding>(&mut buf)
				.map_err(|e| Crypt4GHError::DecryptKeyError(e.to_string()))?;
			output = buf.to_vec();
		...
		"aes256-cbc" => {
			let mut buf = output.clone();
			let dec = CbcDecryptor::<Aes256>::new_from_slices(key, iv)
				.map_err(|e| Crypt4GHError::DecryptKeyError(e.to_string()))?;
			dec.decrypt_padded_mut::<NoPadding>(&mut buf)
				.map_err(|e| Crypt4GHError::DecryptKeyError(e.to_string()))?;
			output = buf.to_vec();
		},
		"3des-cbc" => return Err(Crypt4GHError::BadCiphername("3des-cbc".into())),

...
| [RUSTSEC-2026-0258](https://rustsec.org/advisories/RUSTSEC-2026-0258) (h2 unbounded empty DATA frames) | `h2` 0.3.27 | 2026-08-25 | Transitive via `hyper` 0.14 ← `aws-smithy-http-client` / AWS SDK. Patched `h2` 0.4.19 is already in the lockfile on the `hyper` 1.x path. Advisory patched versions are `>=0.4.16`; no 0.3 release exists. | `aws-sdk-*` drop `hyper` 0.14; then drop the ignore and re-run `cargo deny check advisories`. |
